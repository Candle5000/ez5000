...
require_once("/var/www/functions/xml/skill_form_upd.php");
$group_xml = "/var/www/functions/xml/skill_group.xml";
$form_login_xml = "/var/www/functions/xml/admin_login_form.xml";
$form_add_xml = "/var/www/functions/xml/skill_form_add.xml";
$PAGESIZE = 20;
$table = "skill";
...
if(isset($_SERVER["REQUEST_METHOD"]) == "POST") {
	...
	if(isset($_POST["submit_add"])) {
		$cols = array("id","name","category","learning","cost","recast","cast","text","note","ep","enhance");
		foreach($cols as $col) {
	...
	if(isset($_POST["submit_upd"])) {
		...
		$target = "id=".$id;
		$cols = array("name","category","learning","cost","recast","cast","text","note","ep","enhance");
		foreach($cols as $col) {
	...
	if(isset($data)) {
		$data->select_column("id", $table, "category", $group_id);
		$count = $data->rows();	
	}
...
<hr>
<?php
	$data->select_column("*", $table, "category", $group_id, $page, $PAGESIZE);
	while($row = $data->fetch()){
?>
<hr>
<div>
<?=$form->load_xml_string(xml_skill_form_upd($row))?>
</div>
<?php
	}
?>
