...
require_once("/var/www/functions/xml/quest_form_upd.php");
$group_xml = "/var/www/functions/xml/quest_group.xml";
$form_login_xml = "/var/www/functions/xml/admin_login_form.xml";
$form_add_xml = "/var/www/functions/xml/quest_form_add.xml";
$PAGESIZE = 20;
$table = "quest";
...
if(isset($_SERVER["REQUEST_METHOD"]) == "POST" && !isset($login_err)) {
	...
	if(isset($data)) {
		$data->select_column("id", $table, "id", "BETWEEN $group_id AND ".quest_category_end($category, $group_id));
		$count = $data->rows();
	}
...
<hr>
<?php
	$data->select_column_p("*", $table, "id BETWEEN $group_id AND ".quest_category_end($category, $group_id), $page, $PAGESIZE, "id");
	while($row = $data->fetch()){
?>
<hr>
<div>
<?=$form->load_xml_string(xml_quest_form_upd($row))?>
</div>
<?php
	}
?>
